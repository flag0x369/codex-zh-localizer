...
Codex Zh Localizer 是一个非官方的 OpenAI Codex 中文化 CLI，用来把 Codex 插件市场、插件详情页技能、技能选择列表、prompt 列表和应用连接器说明转换成简体中文。它只处理可恢复的本地插件/技能/prompt 元数据缓存，不修改 Codex App 本体。

适合搜索“Codex 中文化”“Codex 插件市场中文”“OpenAI Codex 中文插件”“Codex plugin marketplace localization”的用户。

![Codex Zh Localizer 安装后效果对比：插件市场、技能说明和应用连接器从英文变成中文可读](assets/codex-zh-localizer-cover.svg)

安装后效果对比：运行 `apply` 后，Codex 插件市场、技能说明、技能选择列表、prompt 描述和应用连接器从英文扫读变成中文可读；示意图不包含真实账号、路径或私有工作区信息。
...
| 技能选择列表仍是英文 | 中文化 `SKILL.md` 和 `agents/openai.yaml` 的展示描述，保留 `name` 和正文 |
| prompt 列表描述仍是英文 | 中文化 `.codex/prompts/*.md` 的 `description`，保留文件名和正文 |
| 应用连接器说明还是英文 | 中文化应用名和连接器描述 |
| Codex 更新或新增技能后中文又没了 | 重新运行 `apply`，幂等恢复支持范围内的中文 |
...
| 技能选择列表描述 | 支持 | `SKILL.md` frontmatter 的 `description` / `metadata.short-description` 和 `agents/openai.yaml` 的 `short_description` |
| Prompt 列表描述 | 支持 | `.codex/prompts/*.md` frontmatter 的 `description` |
| 插件详情页应用 | 支持 | app connector directory cache |
| Codex 前端内置按钮和菜单 | 暂不支持 | 这些通常在 App bundle / `app.asar` 内 |
| 技能 `name`、正文、品牌名、命令、API key | 保留原文 | 避免影响 Codex 触发和执行技能 |
...
skillYaml: 608/608 已中文化
skillMd: 666/666 已中文化
promptMd: 5/5 已中文化
...
| 不翻译技能 `name`、技能正文、prompt 文件名、prompt 正文、品牌名和命令 | 避免影响 Codex 触发、执行和配置 |

每次 `apply` 前都会创建备份，`restore-marketplace`、`restore-components`、`restore-skill-md` 和 `restore-prompts` 会按补丁类型恢复，避免串台。

## 为什么不直接改 app.asar

Codex 前端内置 UI 文案位于 App bundle 资源中，其中包括 `app.asar`。直接 patch App 本体可能影响 macOS App 签名、Codex 自动更新、完整性校验和下次升级后的可恢复性。

当前版本选择更稳的路线：只中文化插件市场、技能描述、prompt 描述和应用连接器这些可恢复的本地元数据。
