...
Codex Zh Localizer 是一个非官方的 OpenAI Codex 中文化 CLI，用来把 Codex 插件市场、插件详情页技能、技能选择列表和应用连接器说明转换成简体中文。它只处理可恢复的本地插件/技能元数据缓存，不修改 Codex App 本体。
...
安装后效果对比：运行 `apply` 后，Codex 插件市场、技能说明、技能选择列表和应用连接器从英文扫读变成中文可读；示意图不包含真实账号、路径或私有工作区信息。
...
skillYaml: 607/607 已中文化
skillMd: 656/656 已中文化
...
| 不翻译技能 `name`、正文、品牌名和命令 | 避免影响 Codex 触发、执行和配置 |

每次 `apply` 前都会创建备份，`restore-marketplace`、`restore-components` 和 `restore-skill-md` 会按补丁类型恢复，避免串台。

## 为什么不直接改 app.asar

Codex 前端内置 UI 文案位于 App bundle 资源中，其中包括 `app.asar`。直接 patch App 本体可能影响 macOS App 签名、Codex 自动更新、完整性校验和下次升级后的可恢复性。

当前版本选择更稳的路线：只中文化插件市场、技能描述和应用连接器这些可恢复的本地元数据。
